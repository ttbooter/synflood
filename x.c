...
int cnipint()
{
    ...
    int arrlen = sizeof(ips)/sizeof(ips[0]);
    //printf("ips len: %d\n", arrlen);

    int j;
    srand(time(0));
    j = rand() % arrlen;
    //printf("got random index: %d\n", j);
    printf("the random ip: %s\n", ips[j]);

    char tmp[strlen(ips[j])];
    char finalip[20] = { 0 };
    char tmpdig[4] = { 0 };
    int i = 0;
    ...
    struct in_addr ip;
    int err = 0;
    err = inet_aton(finalip, &ip);

    if (err) {
        //printf("ip string:\t%s\nip heximal:\t0x%x\nip decimal:\t%d\n", finalip, ip.s_addr, ip.s_addr);
        return ip.s_addr;
    } else {
        printf("inet_aton: string %s convert error\n", finalip);
        return 1370717149;
    }
